...
$tipoOrden   = strtolower($_GET['tipo'] ?? 'asc'); 
$filtroEstado = $_GET['estado']   ?? '';

function limpiar($v, $def = '') { return isset($v) ? trim((string)$v) : $def; }

function crearProductoDesdeRequest(array $src) : Producto {
    $datos = [
        'id'            => isset($src['id']) ? (int)$src['id'] : null,
        'nombre'        => limpiar($src['nombre'] ?? ''),
        'descripcion'   => limpiar($src['descripcion'] ?? ''),
        'estado'        => limpiar($src['estado'] ?? 'disponible'),
        'stock'         => isset($src['stock']) ? (int)$src['stock'] : 0,
        'fechaIngreso'  => limpiar($src['fechaIngreso'] ?? date('Y-m-d')),
        'categoria'     => strtolower(limpiar($src['categoria'] ?? ''))
    ];

    switch ($datos['categoria']) {
        case 'electronico':
        case 'electrónico':
            $datos['garantiaMeses'] = isset($src['garantiaMeses']) ? (int)$src['garantiaMeses'] : 0;
            return new ProductoElectronico($datos);

        case 'alimento':
            $datos['fechaVencimiento'] = limpiar($src['fechaVencimiento'] ?? '');
            return new ProductoAlimento($datos);

        case 'ropa':
            $datos['talla'] = limpiar($src['talla'] ?? '');
...
if ($operacion === 'crear' && !empty($_GET['nombre'])) {
    $nuevo = crearProductoDesdeRequest($_GET);
    $gestor->agregar($nuevo);
    $notificacion = "Producto agregado correctamente.";
    
} elseif ($operacion === 'modificar' && !empty($_GET['id'])) {
    $actualizado = crearProductoDesdeRequest($_GET);
    $ok = $gestor->actualizar($actualizado);
    $notificacion = $ok ? "Producto modificado correctamente." : "No se encontro el producto a modificar.";

} elseif ($operacion === 'eliminar' && !empty($_GET['id'])) {
    $ok = $gestor->eliminar($_GET['id']);
    $notificacion = $ok ? "Producto eliminado correctamente." : "No se encontr0 el producto a eliminar.";

} elseif ($operacion === 'cambiar_estado' && !empty($_GET['id']) && !empty($_GET['nuevo_estado'])) {
    $ok = $gestor->cambiarEstado($_GET['id'], $_GET['nuevo_estado']);
    $notificacion = $ok ? "Estado actualizado correctamente." : "No se pudo actualizar el estado (ID no encontrado).";

} elseif ($operacion === 'editar' && !empty($_GET['id'])) {
    $itemParaEditar = $gestor->obtenerPorId($_GET['id']);
    if (!$itemParaEditar) {
        $notificacion = "No se encontró el producto para editar.";
    }
}
